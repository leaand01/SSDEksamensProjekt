...
def share_with_all_users(mapper, connection, target):  #forsøg tror unødvendig når fjernet user_id i SharedCalcsWithAll
...
def delete_shared_calcs(session, calc_id, user_id):
    ...
    # shared_calcs_to_delete = session.query(SharedCalcsWithAll).filter_by(calc_id=calc_id).all() # forsøg
